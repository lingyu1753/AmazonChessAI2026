...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    while True:
        ...
        num_batches_per_epoch = int(round(samples_per_epoch / batch_size))
        pbar = tqdm(total=num_batches_per_epoch, desc=f"Epoch {num_epochs_this_instance}", unit="batch", dynamic_ncols=True)
        for i in range(sub_epochs):
            ...
            for batch in data_processing_pytorch.read_npz_training_data(
                ...
                if batch_count_this_epoch % print_train_loss_every_batches == 0:
                    ...
                    pbar.set_postfix({
                        "loss": f"{metrics.get('loss_sum', 0):.2f}",
                        "lr": f"{lr_right_now:.2e}",
                    })
        ...
        if rank == 0:
            ...
            epoch_timing_path = os.path.join(traindir, "epoch_times.txt")
            with open(epoch_timing_path, "a", encoding="utf-8") as f:
                f.write(
                    f"epoch={num_epochs_this_instance-1} | "
                    f"time={epoch_elapsed:.1f}s | "
                    f"samples={train_state['global_step_samples']} | "
                    f"data_rows={train_state['total_num_data_rows']} | "
                    f"lr={lr_right_now:.6f} | "
                    f"loss={running_metrics['sums'].get('loss',0):.4f} | "
                    f"pacc1={running_metrics['sums'].get('pacc1',0):.4f} | "
                    f"vsquare={running_metrics['sums'].get('vsquare',0):.4f}\n"
                )

    logging.info("=" * 60)
    logging.info("TRAINING COMPLETE")
    logging.info(f"Total epochs: {num_epochs_this_instance}")
    logging.info(f"Total samples: {train_state['global_step_samples']}")
    logging.info(f"Total data rows: {train_state['total_num_data_rows']}")
    if last_val_metrics and "sums" in last_val_metrics:
        val_sums = last_val_metrics["sums"]
        val_weights = last_val_metrics["weights"]
        summary = {}
        for k in val_sums:
            if k.endswith("_sum"):
                summary[k[:-4]] = val_sums[k] / max(val_weights.get(k, 1), 1)
        logging.info("Final validation: " + ", ".join(f"{k}={v:.4f}" for k, v in sorted(summary.items()) if k in ("pacc1", "vsquare", "loss")))
    logging.info("=" * 60)
