...
def read_npz_training_data(
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for next_file in (npz_files[1:] + [None]):
            ...
            logging.debug(f"Beginning {npz_file} with {num_whole_steps * world_size} usable batches, my rank is {rank}")

            if next_file is not None:
                logging.debug(f"Preloading {next_file} while processing this file")
