...
static void mallocOnDevice(const string& name, int numWeights, void*& deviceBuf, bool useFP16) {
  if(useFP16) {
    ...
    CUDA_ERR(name.c_str(),cudaMalloc((void**)&deviceBuf, halfBytes));
  }
  else {
    size_t floatBytes = numWeights * sizeof(float);
    CUDA_ERR(name.c_str(),cudaMalloc((void**)&deviceBuf, floatBytes));
  }
}
...
struct Buffers {
  ...
  Buffers(CudaHandles* cudaHandles, const Model& m, bool useFP16) {
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&inputBuf, inputBufBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&inputGlobalBufFloat, inputGlobalBufBytesFloat));
    CUDA_ERR("Buffers",cudaMalloc((void**)&inputGlobalBuf, inputGlobalBufBytes));

    CUDA_ERR("Buffers",cudaMalloc((void**)&maskBuf, batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&maskFloatBuf, batchXYFloatBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&maskSumBuf, batchFloatBytes));

    CUDA_ERR("Buffers",cudaMalloc((void**)&trunkBuf, m.trunk->trunkNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&trunkScratchBuf, m.trunk->trunkNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&regularOutBuf, m.trunk->regularNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&regularScratchBuf, m.trunk->regularNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&dilatedOutBuf, m.trunk->dilatedNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&midInBuf, (m.trunk->regularNumChannels + m.trunk->dilatedNumChannels) * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&midScratchBuf, (m.trunk->regularNumChannels + m.trunk->dilatedNumChannels) * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&gpoolOutBuf, m.trunk->gpoolNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&gpoolOutBuf2, m.trunk->gpoolNumChannels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&gpoolConcatBuf, m.trunk->gpoolNumChannels * batchBytes * 3));
    CUDA_ERR("Buffers",cudaMalloc((void**)&gpoolBiasBuf, m.trunk->regularNumChannels * batchBytes));

    CUDA_ERR("Buffers",cudaMalloc((void**)&p1OutBuf, m.policyHead->p1Channels * batchXYFloatBytes)); //need to hold floats in addition to halfs
    CUDA_ERR("Buffers",cudaMalloc((void**)&p1OutBuf2, m.policyHead->p1Channels * batchXYFloatBytes)); //need to hold floats in addition to halfs
    CUDA_ERR("Buffers",cudaMalloc((void**)&g1OutBuf, m.policyHead->g1Channels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&g1OutBuf2, m.policyHead->g1Channels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&g1ConcatBuf, m.policyHead->g1Channels * batchFloatBytes * 3));
    CUDA_ERR("Buffers",cudaMalloc((void**)&g1BiasBuf, m.policyHead->p1Channels * batchFloatBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&p2OutBuf, m.policyHead->p2Channels * batchXYFloatBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&g1PassBuf, m.policyHead->p2Channels * batchFloatBytes));
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&v1OutBuf, m.valueHead->v1Channels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&v1OutBuf2, m.valueHead->v1Channels * batchXYBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&v1MeanBuf, m.valueHead->v1Channels * 3 * batchFloatBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&v2OutBuf, m.valueHead->v2Channels * batchFloatBytes));
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&ownershipBuf, ownershipBufBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&ownershipScratchBuf, ownershipBufBytes));
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&workspaceBuf, bytes));
...
bool NeuralNet::testEvaluateConv(
  ...
  CUDA_ERR("deviceWorkspace",cudaMalloc((void**)&deviceWorkspace, workspaceBytes));
