...
TrunkDesc::TrunkDesc(istream& in, int vrsn, bool binaryFloats) {
  ...
  if(version >= 15) {
    ...
    in >> dummy1 >> dummy2 >> dummy3 >> dummy4 >> dummy5 >> dummy6;
  }
...
PolicyHeadDesc::PolicyHeadDesc(istream& in, int vrsn, bool binaryFloats) {
  ...
  if(version >= 15) {
    ...
    p2Activation = ActivationLayerDesc(in);
    ...
    if(gpoolToPassMul.inChannels != g1BN.numChannels * 3)
      throw StringError(
        name + Global::strprintf(
                 ...
                 g1BN.numChannels * 3));
    if(gpoolToPassMul.outChannels != p1BN.numChannels)
      throw StringError(
        name + Global::strprintf(
                 ": gpoolToPassMul.outChannels (%d) != p1BN.numChannels (%d)",
                 gpoolToPassMul.outChannels,
                 p1BN.numChannels));
    if(gpoolToPassBias.numChannels != p1BN.numChannels)
      throw StringError(
        name + Global::strprintf(
                 ": gpoolToPassBias.numChannels (%d) != p1BN.numChannels (%d)",
                 gpoolToPassBias.numChannels,
                 p1BN.numChannels));
    if(gpoolToPassMul2.inChannels != p1BN.numChannels)
      throw StringError(
        name + Global::strprintf(
                 ": gpoolToPassMul2.inChannels (%d) != p1BN.numChannels (%d)",
                 gpoolToPassMul2.inChannels,
                 p1BN.numChannels));
    if(gpoolToPassMul2.outChannels != 2)
      throw StringError(
        name + Global::strprintf(": gpoolToPassMul2.outChannels (%d) != 2", gpoolToPassMul2.outChannels));
  } else {
...
PolicyHeadDesc& PolicyHeadDesc::operator=(PolicyHeadDesc&& other) {
  ...
  p2Activation = std::move(other.p2Activation);
...
ModelDesc::ModelDesc(istream& in, bool binaryFloats) {
  ...
  if(version >= 13) {
    double td_score_multiplier, scoremean_multiplier, scorestdev_multiplier;
    double lead_multiplier, variance_time_multiplier;
    double shortterm_value_error_multiplier, shortterm_score_error_multiplier;
    in >> td_score_multiplier >> scoremean_multiplier >> scorestdev_multiplier;
    in >> lead_multiplier >> variance_time_multiplier;
    in >> shortterm_value_error_multiplier >> shortterm_score_error_multiplier;
  }

  if(version >= 15) {
    int meta_encoder_version;
    int dummy1, dummy2, dummy3, dummy4, dummy5, dummy6, dummy7;
    in >> meta_encoder_version;
